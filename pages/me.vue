...
const hasPersonalInfo = computed(() => {
  return user.value && (user.value.gender || user.value.age || user.value.langLevel || user.value.idealPartnerDescription);
});

const hasPartnerFilters = computed(() => {
  return user.value && user.value.partnerFilters &&
    (user.value.partnerFilters.gender ||
      (user.value.partnerFilters.minAge && user.value.partnerFilters.maxAge) ||
      (user.value.partnerFilters.langLevel && user.value.partnerFilters.langLevel.length > 0));
});
    ...
    <v-row class="d-flex justify-center">
      <v-col cols="12" md="11" xl="10">
          ...
          <v-row align="center" class="mb-6 mb-md-8">
            <v-col cols="12" md="auto">
              <div class="d-flex align-center">
                ...
                <div>
                  <h1 class="text-h4 font-weight-bold text-grey-darken-3">{{ user.name }} {{ user.surname }}</h1>
                  <p class="text-subtitle-1 text-grey-darken-1">{{ user.email }}</p>
                </div>
              </div>
            </v-col>
            <v-spacer></v-spacer>
              ...
              <v-card-text class="pa-0">
                <div v-if="hasPersonalInfo" class="profile-grid">
                  <div v-if="user.gender" class="profile-grid-item">
                    ...
                    <div class="text-body-1 text-grey-darken-4">{{ formatGender(user.gender) }}</div>
                  </div>
                  <div v-if="user.age" class="profile-grid-item">
                    <div class="text-subtitle-2 text-grey-darken-2">Возраст</div>
                    <div class="text-body-1 text-grey-darken-4">{{ user.age }}</div>
                  </div>
                  <div v-if="user.langLevel" class="profile-grid-item">
                    <div class="text-subtitle-2 text-grey-darken-2">Мой уровень языка</div>
                    <div class="text-body-1 text-grey-darken-4">
                      {{ user.langLevel.name }}
                      <span class="text-caption text-grey-darken-1">({{ user.langLevel.description }})</span>
                    </div>
                  </div>
                  <div v-if="user.idealPartnerDescription" class="profile-grid-item full-width">
                    <div class="text-subtitle-2 text-grey-darken-2">Описание идеального партнера</div>
                    <p class="text-body-1 text-grey-darken-4" style="white-space: pre-wrap;">{{
                ...
                <v-alert v-else type="info" variant="tonal" density="compact" icon="mdi-information-outline">
                  ...
                  <v-btn variant="text" size="small" color="primary" @click="router.push('/about-me-form')"
                    class="ml-2">Заполнить</v-btn>
                </v-alert>
              </v-card-text>
            </v-card>
          </section>
            ...
            <v-card flat color="transparent">
              <v-card-text class="pa-0">
                <div v-if="hasPartnerFilters" class="profile-grid">
                  <div v-if="user.partnerFilters?.gender" class="profile-grid-item">
                    <div class="text-subtitle-2 text-grey-darken-2">Предпочитаемый пол</div>
                    <div class="text-body-1 text-grey-darken-4">{{ formatGender(user.partnerFilters.gender) }}</div>
                  </div>
                  <div v-if="user.partnerFilters?.minAge && user.partnerFilters?.maxAge" class="profile-grid-item">
                    <div class="text-subtitle-2 text-grey-darken-2">Предпочитаемый возраст</div>
                    <div class="text-body-1 text-grey-darken-4">От {{ user.partnerFilters.minAge }} до {{
                      user.partnerFilters.maxAge }}</div>
                  </div>
                  <div v-if="user.partnerFilters?.langLevel && user.partnerFilters.langLevel.length > 0"
                    class="profile-grid-item">
                    <div class="text-subtitle-2 text-grey-darken-2">Предпочитаемые уровни языка</div>
                    <div class="text-body-1 text-grey-darken-4">{{ formatLangLevels(user.partnerFilters.langLevel) }}
                    </div>
                  </div>
                </div>
                <v-alert v-else type="info" variant="tonal" density="compact" icon="mdi-information-outline">
        ...
        <v-container v-else class="d-flex flex-column align-center justify-center fill-height">
          ...
          <p class="text-subtitle-1 text-grey-darken-2 mb-4">Загрузка данных пользователя...</p>
